...
export default function GroundDetails() {
  ...
  function handleSelectSession(session) {
    if (session.status === "booked" || session.status === "Booked") {
  ...
  async function handleWhatsAppBooking() {
    ...
    if (!ground?.admin_phone) {
      alert("Ground owner WhatsApp number is not set");
      return;
    }
...
Please confirm availability.`;
      ...
      alert(err.message || "Failed to create booking request");
    } finally {
          ...
          {slots.map((session) => {
            ...
            return (
              <Card
                ...
                } ${isBooked ? "opacity-80" : "hover:border-pink-400"}`}
